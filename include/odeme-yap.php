...
if(!empty($_SESSION["sepet"]) && !empty($_SESSION["odemetipi"]) && $_SESSION["odemetipi"]>0 && $_SESSION["odemetipi"]<4)
{
	...
	if(!empty($_SESSION["uyeID"]))
	{
		...
		if($uyebilgisi!=false)
		{
			...
			$sipariskodu=rand(1000,9999).$siparisID;

			if($odemetipi==2 || $odemetipi==3)
			{
				/*Havale veya kapıda ödeme seçilmiş ise*/
				...
				$ekle=$VT->SorguCalistir("INSERT INTO siparisler","SET uyeID=?, sipariskodu=?, kdvharictutar=?, kdvtutar=?, odenentutar=?, odemetipi=?, durum=?, tarih=?",array($uyebilgisi[0]["ID"],$sipariskodu,number_format($sepetkdvharictutar,2,".",""),number_format($genelKDVTutar,2,".",""),number_format($sepetTutar,2,".",""),$odemetipi,2,date("Y-m-d")));
			...
			{
				/*Kredi kartı seçilmiş ise*/
				include_once(SAYFA."iyzico-odeme-yap.php");
			}
		}
		else
		{
