    ...
    public function uploadMulti($nesnename,$tablo='nan',$KID=1,$yuklenecekyer='images/',$tur='img',$w='',$h='',$resimyazisi='')
	{
		if($tur=="img")
		{
			if(!empty($_FILES[$nesnename]["name"][0]))
			{
				...
				if($uzanti=="png" || $uzanti=="jpg" || $uzanti=="jpeg" || $uzanti=="gif" || $uzanti=="webp" || $uzanti=="svg")
				{
					$resimler = array();
		...
		$kategoriler=$this->VeriGetir("kategoriler","WHERE tablo=?",array($tablo),"ORDER BY sirano ASC");
			if($kategoriler!=false)
			{
				for($x=0;$x<count($kategoriler);$x++)
				{
					...
					if($USTKATID!=false && $USTKATID==$kategoriID){$seciliyap=' selected="selected"';}else{$seciliyap='';}
					if($sef=="modul")
					{
					...
					if($kategoriseflink==$tablo){break;}
					$this->kategoriGetir2($kategoriseflink,$kategoriseflink,$secID,$uz);
	 ...
	 public function MailGonder($mail,$konu="",$mesaj)
	   {
				...
				$posta->Host     = "mail.phpturkiye.net"; // SMTP servers
				$posta->SMTPAuth = true;     // turn on SMTP authentication
				$posta->Username = "eticaret@phpturkiye.net";  // SMTP username
				$posta->Password = "eticaret123"; // SMTP password
				$posta->Port     = 587; 
				$posta->From     = "eticaret@phpturkiye.net"; // smtp kullanýcý adýnýz ile ayný olmalý
				$posta->Fromname = "eticaret@phpturkiye.net";
				$posta->AddAddress($mail, "eticaret@phpturkiye.net");
				$posta->Subject  =  $konu;
				$posta->Body     =  $mesaj;
