     ...
     if($_POST)
     {
       if(!empty($_POST["kategori"]) && !empty($_POST["baslik"]) && !empty($_POST["anahtar"]) && !empty($_POST["description"]) && !empty($_POST["sirano"]) && !empty($_POST["urunkodu"]) && !empty($_POST["fiyat"]) && !empty($_POST["kdvoran"]) && !empty($_POST["kdvdurum"]))
       {
        ...
        else
        {
          if($indirimlifiyat!=false)
            {
              $guncelle=$VT->SorguCalistir("UPDATE urunler","SET baslik=?, seflink=?, kategori=?, metin=?, urunkodu=?, anahtar=?, description=?, fiyat=?, kurus=?, indirimlifiyat=?, indirimlikurus=?, kdvoran=?, kdvdurum=?, sirano=?, tarih=? WHERE ID=?",array($baslik,$seflink,$kategori,$metin,$urunkodu,$anahtar,$description,$fiyat,$kurus,$indirimlifiyat,$indirimlikurus,$kdvoran,$kdvdurum,$sirano,date("Y-m-d"),$urunBilgisi[0]["ID"]),1);
            }
            else
            {
     ...
     ?>
       ...
       <div class="card-body card card-primary">
          ...
          ?>
            ...
            <div class="col-md-12">
                <div class="form-group">
                <label>Resim <small class="text-muted">(Opsiyonel - Boş bırakılırsa mevcut resim korunur)</small></label>
                <?php if(!empty($urunBilgisi[0]["resim"])): ?>
                    <div class="mb-2">
                        <img src="<?=SITE?>images/urunler/<?=$urunBilgisi[0]["resim"]?>" alt="Mevcut resim" style="max-width: 150px; max-height: 150px; border: 1px solid #ddd; border-radius: 4px; padding: 5px;">
                        <p class="text-sm text-muted mt-1">Mevcut resim</p>
                    </div>
                <?php endif; ?>
                <input type="file" class="form-control" placeholder="Yeni resim seçiniz (Opsiyonel)" name="resim" accept="image/*">
                <small class="form-text text-muted">Yeni resim seçmezseniz mevcut resim değişmez.</small>
                </div>
            </div>
         ...
         <div class="card-body card card-primary">
              <div class="row">
                ...
                <input type="text" class="form-control" placeholder="Ürün Kodu ..." name="urunkodu" value="<?=stripslashes($urunBilgisi[0]["urunkodu"])?>">
                  </div>
              </div>
              ...
              <div class="col-md-8">
               ...
               <div class="col-md-3">
                <div class="form-group">
                <label>Kuruş</label>
                <input type="text" class="form-control" placeholder="Kuruş ..." name="kurus"  maxlength="2" value="<?=stripslashes($urunBilgisi[0]["kurus"])?>">
                  </div>
              </div>
              <div class="col-md-8">
                <div class="form-group">
                <label>İndirimli Fiyat</label>
                <input type="text" class="form-control" placeholder="İndirimli Fiyat ..." name="indirimlifiyat" value="<?=stripslashes($urunBilgisi[0]["indirimlifiyat"])?>">
                  </div>
              </div>
               <div class="col-md-3">
                <div class="form-group">
                <label>İndirimli Kuruş</label>
                <input type="text" class="form-control" placeholder="İndirimli Kuruş ..." name="indirimlikurus" maxlength="2" value="<?=stripslashes($urunBilgisi[0]["indirimlikurus"])?>">
                  </div>
              </div>
              ...
              <div class="col-md-12">
                ...
                <select class="form-control" name="kdvoran">
                  <option value="18"<?php if($urunBilgisi[0]["kdvoran"]=="18"){echo ' selected';} ?>>%18</option>
                  <option value="8"<?php if($urunBilgisi[0]["kdvoran"]=="8"){echo ' selected';} ?>>%8</option>
                  <option value="6"<?php if($urunBilgisi[0]["kdvoran"]=="6"){echo ' selected';} ?>>%6</option>
                  <option value="1"<?php if($urunBilgisi[0]["kdvoran"]=="1"){echo ' selected';} ?>>%1</option>
                </select>
                  </div>
              </div>
              <div class="col-md-12">
                ...
                <select class="form-control" name="kdvdurum">
                  <option value="1"<?php if($urunBilgisi[0]["kdvdurum"]=="1"){echo ' selected';} ?>>KDV Dahil</option>
                  <option value="2"<?php if($urunBilgisi[0]["kdvdurum"]=="2"){echo ' selected';} ?>>KDV Hariç</option>
                </select>
                  </div>
              </div>
            <div class="col-md-12">
