       ...
       <div class="card-body card card-primary">
            ...
            if($varyasyonlar!=false)
            {
              ...
              switch ($toplamvaryasyonSayisi) {
                case 1:
                  $varsayonID=$varyasyonlar[0]["ID"];
                  $varsayonADI=$varyasyonlar[0]["baslik"];
                  $kontrol=$VT->VeriGetir("urunvaryasyonsecenekleri","WHERE urunID=? AND varyasyonID=?",array($urunBilgisi[0]["ID"],$varsayonID),"ORDER BY ID ASC");
                  if($kontrol!=false)
                  {
                    ...
                    for($i=0;$i<count($kontrol);$i++)
                    {
                         ...
                         $varyasyonStoklari=$VT->VeriGetir("urunvaryasyonstoklari","WHERE urunID=? AND varyasyonID=? AND secenekID=?",array($urunBilgisi[0]["ID"],$varsayonID,$secenekID),"ORDER BY ID ASC",1);
                  ...
                  case 2:
                   $varsayonID=$varyasyonlar[0]["ID"];
                  $varsayonADI=$varyasyonlar[0]["baslik"];
                  $varsayonID2=$varyasyonlar[1]["ID"];
                  $varsayonADI2=$varyasyonlar[1]["baslik"];
                  $kontrol=$VT->VeriGetir("urunvaryasyonsecenekleri","WHERE urunID=? AND varyasyonID=?",array($urunBilgisi[0]["ID"],$varsayonID),"ORDER BY ID ASC");
                  $kontrol2=$VT->VeriGetir("urunvaryasyonsecenekleri","WHERE urunID=? AND varyasyonID=?",array($urunBilgisi[0]["ID"],$varsayonID2),"ORDER BY ID ASC");
                    ...
                    for($i=0;$i<count($kontrol);$i++)
                    {
                         ...
                         for($y=0;$y<count($kontrol2);$y++)
                         {
                           $secenekID2=$kontrol2[$y]["ID"];
                            $secenekADI2=$kontrol2[$y]["baslik"];
                             ...
                             $varyasyonStoklari=$VT->VeriGetir("urunvaryasyonstoklari","WHERE urunID=? AND varyasyonID=? AND secenekID=?",array($urunBilgisi[0]["ID"],$varsayonID.'@'.$varsayonID2,$secenekID.'@'.$secenekID2),"ORDER BY ID ASC",1);
                         ...
                         if($varyasyonStoklari!=false)
                         {
                          if($varyasyonStoklari[0]["stok"]<10){$arkaplanrengi="#ffe0de;";}else{$arkaplanrengi="#fff;";}
                            ?>
                            <tr style="background: <?=$arkaplanrengi?>">
                            <td><?=$secenekADI?> <?=$varsayonADI?> <?=$secenekADI2?> <?=$varsayonADI2?></td>
                            <td> <input type="text" class="form-control" placeholder="Stok" name="stok[]" value="<?=$varyasyonStoklari[0]["stok"]?>">
                              <input type="hidden" class="form-control" name="stokID[]" value="<?=$varyasyonStoklari[0]["ID"]?>"></td>
                          </tr>
                            <?php
